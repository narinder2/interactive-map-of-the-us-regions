...
if(isset($_POST['act_type']) && $_POST['act_type'] == 'usa_map_plugin_main_save') {
    update_option('freeusahtml5map_nameFontSize', $_POST['name_font_size'].'px');
    update_option('freeusahtml5map_borderColor', $_POST['borders_color']);
    update_option('freeusahtml5map_nameColor', $_POST['name_color']);
    update_option('freeusahtml5map_mapWidth', $_POST['mapWidth']);
    update_option('freeusahtml5map_mapHeight', $_POST['mapHeight']);
    update_option('freeusahtml5map_statesInfoArea', $_POST['statesArea']);
}



echo "<h2>" . __( 'Map settings', 'freeusa-html5-map' ) . "</h2>";
?>
...
<form method="POST" class="usa-html5-map">
    ...
    <span class="title">Name Font Size: </span><input class="span2" type="text" name="name_font_size" value="<?php echo preg_replace('/[^\d]+/i', '', get_option('freeusahtml5map_nameFontSize')); ?>" />
    <span class="tipsy-q" original-title="Font size of names on the map">[?]</span><br />

    <span class="title">Name Color: </span><input id='color' class="color" type="text" name="name_color" value="<?php echo get_option('freeusahtml5map_nameColor'); ?>" style="background-color: #<?php echo get_option('usahtml5map_nameColor'); ?>" readonly />
